...
Texture3D<float4> _RadianceFieldTexture; 
SamplerState sampler_RadianceFieldTexture; // Unity binds this automatically if named matches texture
...
float3 SampleVoxelGI(float3 worldPos, float3 normal)
{
    ...
    float3 samplePos = worldPos + (normal * _RadianceFieldVoxelSize * 1.1);
    ...
    float3 radiance = _RadianceFieldTexture.Sample(sampler_RadianceFieldTexture, uvw).rgb;
    // DEBUG: Detect NaN/Inf (robust): NaN != NaN, and clamp huge values as Inf check.
    if (any(radiance != radiance) || any(abs(radiance) > 1e20)) {
