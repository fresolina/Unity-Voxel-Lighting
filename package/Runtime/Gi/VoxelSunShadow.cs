...
namespace Lotec.Lighting {
    ...
    public class VoxelSunShadow : MonoBehaviour {
        ...
        [Tooltip("Raymarch mode ONLY: cells at the START of the ray whose hits are ignored, so a " +
                 "surface cannot shadow itself. This replaces biasing the ray origin, which cannot be " +
                 "made to work at any value - too small and the ray starts inside its own cell " +
                 "(voxel-scale acne striping), too large and the origin leaves the structure doing the " +
                 "occluding (a 1-cell normal offset lifts a point out from under an arcade vault and " +
                 "the whole arcade reads sunlit).")]
        [Range(0, 8)][SerializeField] int _raymarchSkipCells = 2;
        ...
        static readonly int s_raymarchSkipCells = Shader.PropertyToID("_BgiRaymarchSkipCells");
        ...
        public void SetGlobals() {
            ...
            Shader.SetGlobalInt(s_raymarchSkipCells, Mathf.Clamp(_raymarchSkipCells, 0, 8));
        ...
        void OnValidate() {
            ...
            _raymarchSkipCells = Mathf.Clamp(_raymarchSkipCells, 0, 8);
