...
namespace Lotec.Lighting {
    ...
    public class MaterialBaker {
        ...
        static readonly int s_outAlbedoIntensity = Shader.PropertyToID("_OutAlbedoIntensity");

        const float kEmissionIntensityMax = 1024.0f;
        ...
        public string Bake(VoxelVolume volume, out Texture3D albedoIntensity, int downscaleFactor = 1) {
            ...
            if (!TryBuildTriangleListWorldWithMaterials(root, out Vector3[] triVerts, out Vector4[] triMatA, out Vector2[] triUVs, out int[] triAlbedoTexIdx, out Texture2D[] albedoTextures, out string triErr))
                return triErr;
            ...
            RenderTexture rtAlbedoIntensity = new RenderTexture(lowRes.x, lowRes.y, 0, RenderTextureFormat.ARGBFloat) {
                dimension = TextureDimension.Tex3D,
                volumeDepth = lowRes.z,
                enableRandomWrite = true,
                wrapMode = TextureWrapMode.Clamp,
                filterMode = FilterMode.Trilinear,
                name = $"{root.name}_Material_AlbedoIntensity"
            };
            rtAlbedoIntensity.Create();
            ...
            try {
                ...
                MaterialBakeCompute.SetTexture(_kernel, s_outAlbedoIntensity, rtAlbedoIntensity);
                // If we have an albedo texture array, create Texture2DArray and copy slices (GPU-side)
                ...
                for (int sm = 0; sm < subMeshCount; sm++) {
                    ...
                    if (mat != null && mat.HasProperty("_EmissionColor")) {
                        ...
                        if (mat.HasProperty("_Emission")) {
                            emissionEnabled = mat.GetFloat("_Emission") > 0.5f;
                        ...
                        if (emissionEnabled) {
                            ...
                            Color emissionLinear = (QualitySettings.activeColorSpace == ColorSpace.Gamma)
                                ? emissionColor.linear
                                : emissionColor;
                            emissionIntensity = Mathf.Max(0f, emissionLinear.maxColorComponent);
                        }
                    }
                    float encodedEmissionIntensity = EncodeEmissionIntensityTo8Bit(emissionIntensity);
                    ...
                    for (int i = 0; i + 2 < t.Length; i += 3) {
                        ...
                        matA.Add(new Vector4(baseColor.r, baseColor.g, baseColor.b, encodedEmissionIntensity));
                        ...
                        // Resolve albedo texture for this material
        ...
        static float EncodeEmissionIntensityTo8Bit(float emissionIntensity) {
            float clamped = Mathf.Clamp(emissionIntensity, 0f, kEmissionIntensityMax);
            float encoded = Mathf.Log(1.0f + clamped, 2.0f) / Mathf.Log(1.0f + kEmissionIntensityMax, 2.0f);
            float quantized = Mathf.Round(encoded * 255.0f) / 255.0f;
